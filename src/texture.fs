...
uniform float gaussWeight[(3 + 1) * (3 + 1)]; 
...
void main()
{
	...
	for (int i = -3; i < 3 + 1; i++) {
		for (int j = -3; j < 3 + 1; j++) {
			...
			alphaSum += gaussWeight[abs(i) * (3 + 1) + abs(j)] * texture(ourTexture1, vec2(GoodsTexCoord.x + i_float/goods_width, GoodsTexCoord.y + j_float/goods_height)).a;
		}
	}
	...
	for (int i = -3; i < 3 + 1; i++) {
		for (int j = -3; j < 3 + 1; j++) {
			...
			gaussedColor += gaussWeight[abs(i) * (3 + 1) + abs(j)] * color;
		}
	}
	...
	FragColor = vec4(mix(gaussedColor * lightRatio, originColor, pow(gaussRatio, 3)), 1.0);
}
